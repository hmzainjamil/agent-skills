...
> Modular agent skills library for Claude Code
...
|---------|-------------|--------|
| Core — Modular foundation for skills workflows | Core foundation — the primary abstraction this repo builds on | [docs](#) |
| Execution — task decomposition and skills routing | Execution layer — how tasks get decomposed and routed | [docs](#) |
| Integration — Claude Code + agent connectivity | Integration layer — connecting to external tools and APIs | [docs](#) |
...
```bash
# Quick start
python3 main.py --task "skills task here"

# Power user shortcut
python3 main.py --skills --fast
...
Use agent-skills to automate modular agent skills library for claude code. Perfect for agencies, freelancers, and AI-first teams running skills workflows at scale.
...
```bash
# Add to CLAUDE.md
echo "Auto-activate: skills, agent" >> ~/.claude/CLAUDE.md

# Or load skill directly
/load-skill agent-skills
```
...
{
  "hooks": {
    "PostToolUse": [{"matcher": "skills", "command": "python3 ~/repos/agent-skills/main.py"}]
  }
}
```
...
```bash
# Fork and clone
gh repo fork hmzainjamil/agent-skills --clone
cd agent-skills
